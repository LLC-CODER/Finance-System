...
/**
 * 支出模块 1、分页及条件查询 2、添加支出 3、修改支出 4、删除支出
 */
@WebServlet("/payOut")
public class PayOutServlet extends HttpServlet {
	...
	private void updatePayOut(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// 非空判断
		if (StringUtil.isEmpty(outName) || StringUtil.isEmpty(outTypeId) || StringUtil.isEmpty(accountId)
				|| StringUtil.isEmpty(money) || StringUtil.isEmpty(payOutId)) {
			// 提示失败
			response.getWriter().write("0");
			return;
		}
		...
		/*
		 * select count(1) from account where id = 8 and money >= 2701 select
		 * count(1) from account where id = 8 and money +100 >= 2800
		 */
		long count = 0;
		try {
			// 获取连接
	...
	private void addPayOut(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// 非空判断
		if (StringUtil.isEmpty(outName) || StringUtil.isEmpty(outName) || StringUtil.isEmpty(outName)
				|| StringUtil.isEmpty(outName)) {
			result.setCode(0);
			result.setMsg("添加失败");
			Gson gson = new Gson();
	...
	private void queryPayOutByPages(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// 查询总数的sqlCount
		String sqlCount = "select count(1) " + "from payout p " + "INNER JOIN account a on p.accountId = a.id "
				+ "INNER JOIN outtype o on p.outTypeId = o.id " + "INNER JOIN outtype ot on o.pid = ot.id "
				+ "where a.uid = ?";
		// 查询每页显示的数据的sqlPage
		String sqlPage = "select ot.id as parentId,p.id,p.outName,p.outTypeId,p.money,p.accountId,p.createTime,p.updateTime,p.remark,a.accountName,CONCAT(ot.typeName,'-',o.typeName) as typeName "
				+ "from payout p " + "INNER JOIN account a on p.accountId = a.id "
				+ "INNER JOIN outtype o on p.outTypeId = o.id " + "INNER JOIN outtype ot on o.pid = ot.id "
				+ "where a.uid = ?";
